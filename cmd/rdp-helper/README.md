...
从网页点「远程桌面」走 `semaphore-rdp://` 协议时，Helper **会隐藏控制台窗口**（进程仍在后台等待 mstsc 结束，以便 SOCKS 转发与凭据清理）；只有双击 / 交互 `rdp>` 模式才显示命令行。Helper 在 **mstsc 启动 / 退出** 时回调 Semaphore（`POST /api/rdp/launch-events`），用于审计连接时长。
