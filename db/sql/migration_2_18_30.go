...
type migration_2_18_30 struct {
	db *SqlDb
}

func (m migration_2_18_30) PreApply(tx *gorp.Transaction) error {
	switch m.db.Sql().Dialect.(type) {
	case gorp.SqliteDialect:
		return m.sqliteDropStopTemplateColumns(tx)
	case gorp.MySQLDialect:
		m.dropMySQLFK(tx, "project__device_settings", "stop_template_id")
		return nil
	case gorp.PostgresDialect:
		_, _ = tx.Exec(m.db.PrepareQuery(
			`alter table "project__device_settings" drop constraint if exists "project__device_settings_stop_template_id_fkey"`))
		return nil
	default:
		return nil
	}
...
func (m migration_2_18_30) dropMySQLFK(tx *gorp.Transaction, table, column string) {
...
func (m migration_2_18_30) sqliteDropStopTemplateColumns(tx *gorp.Transaction) error {
	settingsHasCol, err := m.sqliteTableHasColumn(tx, "project__device_settings", "stop_template_id")
	if err != nil {
		return err
	}
	profileHasCol, err := m.sqliteTableHasColumn(tx, "project__device_profile_settings", "stop_template_id")
	if err != nil {
		return err
	}
	if !settingsHasCol && !profileHasCol {
		return nil
	}
...
CREATE TABLE ` + "`project__device_profile_settings_new`" + ` (
  ...
  ` + "`check_restart_template_id`" + ` integer null,
  ` + "`config_template_id`" + ` integer null,
...
INSERT INTO ` + "`project__device_profile_settings_new`" + ` (
  ` + "`project_id`" + `, ` + "`profile_id`" + `, ` + "`discover_template_id`" + `, ` + "`restart_template_id`" + `,
  ` + "`status_template_id`" + `, ` + "`redeploy_template_id`" + `, ` + "`check_restart_template_id`" + `,
  ` + "`config_template_id`" + `, ` + "`resend_data_template_id`" + `, ` + "`default_inventory_id`" + `,
...
func (m migration_2_18_30) sqliteTableHasColumn(tx *gorp.Transaction, table, column string) (bool, error) {
