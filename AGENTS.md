...
- **Device operation split**: `device_redeploy` — check target zip, copy if missing, then config+start. Profile templates: `redeploy_template_id`, `check_restart_template_id` (scheduled refresh). See `docs/plan-split-device-redeploy.md`.
