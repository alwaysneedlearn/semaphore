...
- **WinRM ping 建连失败**：`win_ping` UNREACHABLE 会跳过该主机后续任务；须在 **`winrm_connect_one_attempt.yml`** 每次 ping 失败后及登记回调前执行 **`meta: clear_host_errors`**，再 include **`tasks/semaphore_callback_winrm_connect_failed.yml`**（`device_status: unhealthy`，`end_host`）。
