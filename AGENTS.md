...
- **Task log:** Running or viewing a task opens a **modal dialog** (not a new tab). The dialog title shows template name and task name as plain text. Use **Open in new tab** to open `/project/{id}/history/{taskId}` (no sidebar; names are not links). Legacy `?t=` still opens the dialog.
