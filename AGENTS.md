...
- **Device operation split**: `device_redeploy` — check target zip, copy if missing, then config+start. Profile templates: `redeploy_template_id`, `check_restart_template_id` (bind for manual/API; periodic check_restart via Semaphore Schedules). See `docs/plan-split-device-redeploy.md`.
