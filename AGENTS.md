...
- **Bulk device actions** enqueue extra-vars `configs_by_host` (config map keyed by **both** device IP and hostname) so Ansible `inventory_hostname` (usually IP) resolves merged config; single-device actions use `device` plus `config` — playbooks under `cursor-playbooks/<device_type>/` (e.g. `neware/`) normalize both via `_semaphore_device_rows`. Actions: `stop`, `restart`, **`redeploy`**, `status` (no separate **start** — use **restart** or **redeploy**).
- **Device operation split**: `device_redeploy` — check target zip, copy if missing, then config+start. Profile templates: `redeploy_template_id` (and status/restart/resend as needed). **Check-restart is not bound on Device types** (no device-list button); run `device_check_restart.yml` via Semaphore **Schedules**. See `docs/plan-split-device-redeploy.md`.
