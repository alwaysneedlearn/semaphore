...
- **WinRM ping 建连失败**：`win_ping` 须 **`ignore_unreachable: true`**；bulk 在 **`windows_hosts` play 的 `post_tasks`**（`delegate_to: localhost`）执行，并含 **`semaphore_callback_winrm_fallback_missing_rows`**，避免 exit 4 时未 PUT、UI 停在 `checking`。
