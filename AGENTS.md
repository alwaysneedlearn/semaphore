...
- **Patrol all**: Marks every device `checking`, then runs the configured **Status** template. Clear `checking` by having that playbook call `PUT /api/project/{id}/devices/status/bulk` (Bearer token). Example: `examples/playbooks/device_status_patrol_with_callback.yml` — set template ENV `SEMAPHORE_PROJECT_ID`, `SEMAPHORE_API_TOKEN`, optionally `SEMAPHORE_URL`.
