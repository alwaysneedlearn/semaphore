...
- **Patrol all**: Marks every device `checking`, then runs the configured **Status** template. Playbooks under `cursor-playbooks/` (e.g. `device_status.yml`) bulk-callback when the template has **`SEMAPHORE_API_TOKEN`** (and **`SEMAPHORE_URL`** if the controller is not co-located with Semaphore). **`semaphore_project_id`** is injected into task extra-vars by the API; unreachable hosts must still get a callback row — see `device_status.yml` / `cursor-playbooks/README.md`. Callback payloads include **`ip`** for bulk matching; **`rdp_status` / `winrm_status` / `api_status` are omitted** from patrol/action callbacks so **Probe** (or discovery import) remains authoritative for **TCP** port badges — run **Probe** after a template if you need those refreshed immediately.
...
- **Device ports**: Each device has **`rdp_port`** (default 3389), **`ansible_port`** (WinRM HTTP default 5985, overridable per device and via project defaults), and **`api_port`** (default **9002** for TCP probes and HTTP calls in playbooks). In the DB, **`rdp_status` / `winrm_status` / `api_status`** reflect **TCP reachability** from the **Probe** action (or discovery import), not from patrol/start/restart/stop playbooks — those templates only bulk-update **`device_status`** (+ **`abnormal_reason`**) unless you add keys back. The **Probe** action updates **RDP, WinRM, and API** TCP reachability plus `last_updated` only. Generated `windows_hosts` inventory includes **`api_port`** when the device has a valid port; playbooks resolve HTTP **`api_port`** as **inventory host var → Variable Group `API_PORT` → 9002**. Task extra-vars for `devices` arrays include `api_port` alongside `rdp_port` and `ansible_port`. Discovery **import** clears port fields to zero before upsert so normalized defaults do not overwrite stored per-device ports; new rows still get defaults in `CreateDevice`.
