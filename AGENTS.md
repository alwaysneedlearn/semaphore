...
- **Patrol all**: Marks every device `checking`, then runs the configured **Status** template. Playbooks under `cursor-playbooks/` (e.g. `device_status.yml`) include a bulk callback when template ENV has `SEMAPHORE_PROJECT_ID`, `SEMAPHORE_API_TOKEN`, and optionally `SEMAPHORE_URL` — see `cursor-playbooks/README.md`.
