...
- New SQL migrations under `db/sql/migrations/` must also be appended to `commonScripts` in `db/Migration.go`; otherwise `./bin/semaphore migrate` will never apply them (e.g. `check_restart_redeploy_template_id` requires **v2.18.21**).
...
- **Bulk device actions** enqueue extra-vars `configs_by_host` (config map keyed by **both** device IP and hostname) so Ansible `inventory_hostname` (usually IP) resolves merged config; single-device actions use `device` plus `config` — playbooks under `cursor-playbooks/<device_type>/` (e.g. `neware/`) normalize both via `_semaphore_device_rows`. Actions: `start`, `stop`, `restart`, **`redeploy`**, `status`.
- **Device operation split**: `device_start` always writes config (no zip). `device_redeploy` only — check target zip, copy if missing, then config+start. Profile templates: `redeploy_template_id`, `check_restart_template_id` (scheduled refresh). See `docs/plan-split-device-redeploy.md`.
...
- **TDengine**: No server-side sync. After bulk PUT, playbooks run **`tasks/semaphore_tdengine_publish_from_bulk.yml`** when the template Variable Group sets **`TDENGINE_URL`** (optional **`TDENGINE_USER`**, **`TDENGINE_PASSWORD`**, **`TDENGINE_DATABASE`**, **`TDENGINE_STATUS_TABLE`**). See `docs/tdengine-setup.md`. Applies to all `neware` device action playbooks (single + bulk); not discovery.
