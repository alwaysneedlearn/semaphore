...
- **WinRM ping 建连失败**：`tasks/winrm_ensure_reachable.yml` 在重试用尽后 include **`tasks/semaphore_callback_winrm_connect_failed.yml`**（`device_status: unhealthy`，`end_host`），避免 bulk 漏写、UI 停在 `checking`。
