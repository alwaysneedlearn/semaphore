...
- **`device_status.yml`**, **`device_restart.yml`**, **`device_check_restart_redeploy.yml`**: **`api_port`** for INI **`ReportApiSettings.ServerPort`** and for **BTSClient 数据上报状态 API** (device `api_port` → **`API_PORT`** → **9002**). **`EXE_DIR`** → **`exe_dir_preferred`** (default **`D:\Program Files\NEWARE`**). First tasks: **`winrm_ensure_reachable`**, **`deploy_sem_windows_helper_scripts`**, **`resolve_exe_dir_windows`**. **Health gate** (process running): **`tasks/health_gate_upload_status_api_then_log.yml`** — **`POST http://{ip}:{port}`** with **`CallType=API_STATUS_CALL_TYPE`** (default **1**), retries **`API_STATUS_RETRIES`** / **`API_STATUS_RETRY_DELAY`**; **`ExecResultData==3`** (`EXPORT_STARTED`) → healthy, **skip log keyword**; else fall back to **`log_health_check_windows.yml`**. **`check_restart_redeploy`** passes **`log_health_use_tail: true`** and **`LOG_HEALTH_TAIL_LINES`** (default **8000**).
- **`device_restart.yml`**: the shared vars above plus `ZIP_PATH`, `EXE_ARGS`, `CONFIG_FILE_NAME`, `EXPORT_NOT_STARTED` (**1**), `EXPORT_STARTING` (**2**), `EXPORT_STARTED` (**3**), `POLL_RETRIES` (default **3**), `POLL_DELAY` (default **10**), **`RESTART_DELAY`** (default **30**), **`PROCESS_VERIFY_POLL_SECONDS`** (default **5**). **Writes** `HisDataFromTime`/`HisDataToTime` (`neware_include_his_data_resend: true`): from device/default `SystemConfig` if set, else env `HIS_DATA_FROM_TIME` / empty → **today** / **tomorrow** (`yyyy-MM-dd`). UI-picked range still uses **`device_resend_data.yml`**.
- **`device_resend_data.yml`**: UI **Resend data** action — sets `HisDataFromTime`/`HisDataToTime` from `resend_params`, then stop → INI patch → start → verify (`neware_include_his_data_resend: true`).
- **`device_check_restart_redeploy.yml`**: **API-first pre-gate** (Tail log fallback) like **`device_restart`**. If process **not** running → **`need_reconfigure: true`** immediately.
...
| `TDENGINE_TIMEZONE` | IANA tz for `updated_time` / `check_time`; default `Asia/Shanghai` |
