...
- **`device_status.yml`**, **`device_restart.yml`**, **`device_stop.yml`**, **`device_check_restart_redeploy.yml`** — play1 在任务或 **`post_tasks`** 登记每台 **`semaphore_callback_row`**；**bulk PUT 与状态总览仅在独立 `hosts: localhost` 第二 play**（`strategy: free` 下不可在 play1 `run_once`，避免第一台抢跑）。**已移除** **`semaphore_bulk_put_immediate.yml`** 的 include。`windows_hosts` play 使用 **`strategy: free`**，批量选中设备时一台卡住不会堵住其它台的 WinRM 任务。
