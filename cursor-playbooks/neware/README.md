...
- **`device_check_restart.yml`**: **API-first pre-gate** (Tail log fallback) like **`device_restart`**. If process **not** running → **`need_reconfigure: true`** immediately; when restart runs, HisData is patched like ordinary restart.
