...
- **`device_redeploy.yml`**: after package ensure (copy/extract only if target **exe** missing), **always** updates a desktop shortcut to `exe_path` with **Run as administrator** (`tasks/update_desktop_shortcut_windows.yml` / `files/sem_update_desktop_shortcut_windows.ps1`). Default desktop = **Public Desktop**; override with **`SHORTCUT_DESKTOP`**, name with **`SHORTCUT_NAME`** (default `<exeBase>.lnk`). Shortcut failure does not fail the play (`failed_when: false`). Then stop → INI → start verify.
