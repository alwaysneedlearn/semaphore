...
- **`device_restart.yml`**: the shared vars above plus `ZIP_PATH`, `EXE_ARGS`, `CONFIG_FILE_NAME`, `EXPORT_NOT_STARTED` (**1**), `EXPORT_STARTING` (**2**), `EXPORT_STARTED` (**3**), `POLL_RETRIES` (default **3**), `POLL_DELAY` (default **10**), **`RESTART_DELAY`** (default **30**), **`PROCESS_VERIFY_POLL_SECONDS`** (default **5**). **Does not** write `HisDataFromTime`/`HisDataToTime` (`neware_include_his_data_resend: false`); use **`device_resend_data.yml`** for historical resend via INI + restart.
