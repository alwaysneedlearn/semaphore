...
- **`device_check_restart_redeploy.yml`**: extends **`device_restart.yml`** with the same **log-only pre-gate** as **`device_start`** (Tail + recent window when process is running). If **not** running → **`need_reconfigure: true`** immediately. Reconfigure + **`start_verify_after_reconfig`** match **`device_restart.yml`**.
