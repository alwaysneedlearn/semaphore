...
**Semaphore vs manual test:** WinRM runs `sem_land_gui_installer_interactive.ps1`, which launches `sem_land_gui_installer_worker.ps1` via **Interactive** scheduled task (`scheduled_highest`, fallback `schtasks /IT`). **Manual** `worker.ps1` in RDP skips that layer. One worker per redeploy (lock file). Error **`0x800710E0`** — keep **Lenovo RDP logged on** (session Active).
