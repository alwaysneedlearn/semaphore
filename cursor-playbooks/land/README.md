...
**Semaphore vs manual test:** WinRM runs `sem_land_gui_installer_interactive.ps1`, which writes JSON config / install log / trace under the **same directory as the deployed scripts** (`C:\Windows\Temp\`). It generates a per-run job script (`sem_land_install_job_{ts}.ps1`) and launches it via **Interactive** scheduled task (`Register-ScheduledTask`, Highest) — **no extra CLI args on the task** (avoids Task Scheduler quoting bugs). **Manual** `worker.ps1` in RDP skips that layer. One worker per redeploy (lock file). Error **`0x800710E0`** — keep **Lenovo RDP logged on** (session Active).
...
| `LAND_INSTALL_DLG3_TITLE` / `_BUTTON` | `LHBTS 安装` / `确定` | Step 3 — same window as step 1; coord **88%, 93%** when only **确定** remains |
