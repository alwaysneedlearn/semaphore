...
**Semaphore vs manual test:** WinRM runs `sem_land_gui_installer_interactive.ps1`, which must launch `sem_land_gui_installer_worker.ps1` in the **logged-in desktop user's session** (scheduled task, then **one** `user_session` fallback only). **Manual** `powershell -File sem_land_gui_installer_worker.ps1` in RDP skips that layer. Only **one** worker/installer run per redeploy (lock file + no multi-mode spam). Each wizard button is clicked **once** per step (`BM_CLICK` only, 500ms poll). Error **`0x800710E0`** means the worker never started — keep **Lenovo RDP logged on** when redeploy runs.
...
| `LAND_INSTALL_DLG3_TITLE` / `_BUTTON` | `LHBTS 安装` / `确定` | Step 3 — clicks only when **升级/卸载 gone** (final single 确定); coord **83%, 93%** |
