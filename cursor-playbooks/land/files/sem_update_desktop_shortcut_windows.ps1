# LAND: ensure desktop shortcut has Run as administrator.
# If .lnk exists → only set admin bit (preserve TargetPath), matching:
#   $b=[IO.File]::ReadAllBytes($p); $b[0x15]=$b[0x15]-bor 0x20; ...
# If missing → create pointing at SEMAPHORE_SHORTCUT_EXE_PATH.
#
# Env:
#   SEMAPHORE_SHORTCUT_EXE_PATH     — target exe when creating (redeploy exe_path)
#   SEMAPHORE_SHORTCUT_PROFILE_USER — DOMAIN\user or short name (from explorer GetOwner)
#   SEMAPHORE_SHORTCUT_DESKTOP      — optional explicit desktop dir (overrides profile resolve)
#   SEMAPHORE_SHORTCUT_NAME         — default 监控软件.lnk
...
if ($linkName.Length -eq 0) { $linkName = '监控软件.lnk' }
