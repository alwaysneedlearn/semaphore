...
- **`device_status.yml`**, **`device_start.yml`**, **`device_restart.yml`**, **`device_stop.yml`**, **`check_restart_redeploy.yml`** — play1 **`post_tasks`** 每台登记 **`semaphore_callback_row`**；**bulk PUT 只在独立 `hosts: localhost` 第二 play**（避免 post_tasks 里 `run_once` bulk 在第一台主机结束时抢跑，批量重启时成功机未写入 healthy）。可选 **`semaphore_bulk_put_immediate.yml`**（单条 PUT，常 skip）。**`device_stop.yml`** 仍仅在 post_tasks bulk（步骤短、无第二 play）。
