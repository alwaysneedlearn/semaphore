...
- **`device_status.yml`**, **`device_start.yml`**, **`device_restart.yml`**, **`device_stop.yml`**, **`check_restart_redeploy.yml`** — **`post_tasks`** on `windows_hosts`（`run_once` + `delegate_to: localhost`）运行 `tasks/semaphore_bulk_put_from_hostvars.yml`（内含 WinRM ping 失败 **fallback**），避免 play 因 unreachable **exit 4** 时 Semaphore 跳过原第二 play、bulk 未 PUT。
