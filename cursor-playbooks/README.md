...
- **`api_status`** (playbook bulk callback): **`online`** if the app **`POST`** in that run returns **HTTP `200`**, regardless of JSON business fields (`ResponeResultCode`, `ExecResultData`, etc.). **`offline`** if a **`uri`** ran for that host but **no** qualifying response was `200` (or the task shows a non-200 / failed connection). **`device_status.yml`**: uses **`api_status_result.status == 200`** on the patrol status `POST` (running hosts only; `NOT_RUNNING` → **`offline`**). **`device_start` / `device_restart` / `check_restart_redeploy`**: **`online`** if **`api_check_after` or `api_start_result`** has status `200`; **`offline`** if at least one of those `uri` results exists and neither is `200`; **empty** if neither `uri` ran (e.g. start-verify block skipped) so bulk **does not** overwrite **`api_status`**. **`device_stop.yml`** still forces **`api_status: offline`** after stop.
...
- The Semaphore API also **rejects inconsistent rows**: `PUT …/devices/status/bulk` coerces **`device_status` away from `healthy` when the merged `api_status` is `offline`** (payload may omit **`api_status`** on paths that skip protocol updates; **`device_stop.yml`** now **always** sends **`api_status: offline`**).
