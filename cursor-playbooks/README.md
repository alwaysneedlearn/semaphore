...
- **`device_discovery.yml`** — **`PUT /api/project/{id}/devices/discovery/results`** when **`SEMAPHORE_API_TOKEN`** is set (`task_id` + `devices` array). Rows are **upserted in DB by IP** (`project__device_discovery_host`). UI **`GET .../discovery/results`** (persisted list) or **`?task_id=`** after a scan. Import to inventory is **Import selected** → **`discovery/import`** (with **`device_profile_id`**).
