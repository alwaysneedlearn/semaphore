...
**WinRM 偶发 UNREACHABLE**：所有 `hosts: windows_hosts` 的 play 在其它 `win_*` 任务之前执行 **`tasks/winrm_ensure_reachable.yml`**（`win_ping` 成功即停止后续尝试；第 2 次起先 `reset_connection` 再 ping）。Variable Group：**`WINRM_CONNECT_RETRIES`**（默认 **4**）、**`WINRM_CONNECT_RETRY_DELAY`**（默认 **5** 秒）。**`meta` 任务不能写 `when`**（会 WARNING 且条件无效），重置连接放在单独 include 文件里。若日志出现 **`ansible_winrm_read_timeout unsupported by pywinrm`**，属旧 pywinrm，可升级 runner 依赖或忽略（不影响 ping）。建连仍失败时 **`meta: clear_host_errors`**。
