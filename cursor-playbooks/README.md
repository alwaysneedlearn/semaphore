...
- **`device_discovery.yml`** — **`PUT /api/project/{id}/devices/discovery/results`** when **`SEMAPHORE_API_TOKEN`** is set (`task_id` + `devices` array). UI polls **`GET .../discovery/results?task_id=`**; persistence into the device list is still **Import selected** → **`discovery/import`** (with **`device_profile_id`**).
