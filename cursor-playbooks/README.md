...
- **`api_status`** (playbook bulk callback): **`device_status.yml` (Patrol)**: **`online`** when **`need_reconfigure` is false** (same dual-check as **`NORMAL`** / **`device_status: healthy`**) **or** when the status `POST` returned **HTTP `200`** even if still abnormal (port reachable). **`offline`** for `NOT_RUNNING` / unreachable / abnormal with no HTTP `200`. **`device_start` / `device_restart` / `check_restart_redeploy`**: **`online`** if **`api_check_after` or `api_start_result`** has status `200`; **`offline`** if at least one of those `uri` results exists and neither is `200`; **empty** if neither `uri` ran so bulk **does not** overwrite **`api_status`**. **`device_stop.yml`** still forces **`api_status: offline`** after stop.
