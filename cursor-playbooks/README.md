...
**WinRM 偶发 UNREACHABLE**：所有 `hosts: windows_hosts` 的 play 在其它 `win_*` 任务之前执行 **`tasks/winrm_ensure_reachable.yml`**（`ansible.windows.win_ping` + `reset_connection` 重试）。Variable Group：**`WINRM_CONNECT_RETRIES`**（默认 **4**）、**`WINRM_CONNECT_RETRY_DELAY`**（默认 **5** 秒）。超时见 **`group_vars/windows_hosts.yml`**。建连仍失败时会 **`meta: clear_host_errors`**，避免主机一直被标记 unreachable 导致后续任务只 `...ignoring` 却不再真正连 WinRM。Semaphore 模板请将 playbook 目录设为含 **`ansible.cfg`** 的 `cursor-playbooks` 根目录（或把其中超时变量写入 inventory）。
