...
**WinRM 偶发 UNREACHABLE**：所有 `hosts: windows_hosts` 的 play 在其它 `win_*` 任务之前执行 **`tasks/winrm_ensure_reachable.yml`**，随后 **`tasks/deploy_sem_windows_helper_scripts.yml`** 将 `files/sem_*.ps1` **整目录一次**复制到 `C:\Windows\Temp\`（每台主机每 play 仅一次；`stop` / `collect` / `resolve` / `reconfig_start` 不再各自 `win_copy`）。（`win_ping` 成功即停止后续尝试；第 2 次起先 `reset_connection` 再 ping）。长 play 内在重配流程、日志检查/轮询前可 **`include_tasks: tasks/winrm_refresh_midplay.yml`**（`winrm_force_reconnect: true`，重新 ping）。DEBUG 使用 **`_winrm_ping_last_success`**（避免 loop skip 覆盖 `last_ping`）。**`meta` 不能写 `when`**：`clear_host_errors` / `reset_connection` 放在 **block** 或单独 include 里。Variable Group：**`WINRM_CONNECT_RETRIES`**（默认 **4**）、**`WINRM_CONNECT_RETRY_DELAY`**（默认 **5** 秒）。若日志出现 **`ansible_winrm_read_timeout unsupported by pywinrm`**，属旧 pywinrm，可升级 runner 依赖或忽略（不影响 ping）。建连仍失败时在 block 内 **`meta: clear_host_errors`**。
