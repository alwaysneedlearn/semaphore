...
### `semaphore_callback_row` and protocol vs business status

- **TCP port columns (`rdp_status`, `winrm_status`, `api_status`)** in the Semaphore DB are meant to match **real reachability** from **`POST …/devices/{id}/probe`** (server-side TCP dial) or **discovery import** (scan). Action playbooks (`device_status.yml`, `device_start.yml`, `device_restart.yml`, `device_stop.yml`, `check_restart_redeploy.yml`) therefore **omit** those keys from **`semaphore_callback_row`**. **`PUT …/devices/status/bulk`** only updates a field when it is **non-empty** in the JSON payload (`api/projects/devices.go`); omitted keys **preserve** the last probe/import values.
- **Business outcome** is carried only by **`device_status`** (healthy / unhealthy / checking) plus **`abnormal_reason`** in the callback. After start/stop/restart/patrol, run **Probe** (or rely on scheduled probes) if you need the UI’s port badges refreshed immediately.
- **`device_status.yml` (Patrol):** **`device_status`** still follows **`need_reconfigure`** (same rule as the printed “NORMAL / 异常” summary). Patrol **does not** write **`api_status`**, so **`CoerceDeviceStatusIfAPIOffline`** uses the **stored** (probe) **`api_status`** together with the new **`device_status`** — e.g. you cannot stay **healthy** if the **TCP** API probe is **offline** even when the playbook would otherwise report healthy.
- **Patrol / `device_status.yml` — log task unreachable:** **`检查日志上报状态`** uses **`ignore_unreachable: true`** so a mid-play WinRM drop still produces **`semaphore_callback_row`**; **`need_reconfigure`** is forced and **`abnormal_reason`** records **`WinRM unreachable during log status check`**.
...
- The Semaphore API also **rejects inconsistent rows**: `PUT …/devices/status/bulk` coerces **`device_status` away from `healthy` when the device’s merged `api_status` is `offline`** (payload may omit `api_status`, in which case the **existing stored** value from **Probe** is used).
