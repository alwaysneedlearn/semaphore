...
- **`device_status.yml`**, **`device_start.yml`**, **`device_restart.yml`**, **`device_stop.yml`**, **`check_restart_redeploy.yml`** — 登记 **`semaphore_callback_row`** 后 **`tasks/semaphore_bulk_put_immediate.yml`**（单条 PUT，**不依赖 post_tasks**；全员 `end_host` 时 Semaphore 常只打 PLAY RECAP）。另有 **`post_tasks`** + 独立 **`hosts: localhost`** play 批量汇总成功主机。
