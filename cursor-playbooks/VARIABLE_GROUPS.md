...
目标机无 `LIMS_HIST_EXE` 时，从控制器复制：`LIMS_HIST_PKG_DIR`/`LIMS_HIST_PKG_NAME` → `/root/software/dahua/lims-hist.exe`（或 `LIMS_HIST_SRC` 完整路径）。重发默认 `-state-dir C:\Apps\lims-hist\state` 和 **`-mtime-prefilter`**，**后台拉起**（Semaphore 不等待）；进程跑完自己退出。
