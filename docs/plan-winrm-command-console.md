...
用户在设备上执行的 WinRM 命令需要 **留痕**（谁、何时、对哪台机、执行了什么、结果如何），便于排障与合规。实现上有两条路：

| 方案 | 做法 | 优点 | 缺点 |
|------|------|------|------|
| **A — 应用日志** | 写入 Semaphore 服务端日志（结构化 JSON：`user_id`、`device_id`、`command` 或 hash、`exit_code` 等） | Phase 1 实现快，不改库表 | 不能在 UI 里查历史；日志轮转后难追溯；合规场景较弱 |
| **B — 数据库审计表** | 新建表如 `project__device_winrm_exec_log`，可选在设备详情展示「最近命令」 | 可查询、可导出、适合审计 | 需迁移 + API；要定 stdout 存多少（摘要/截断/不存） |
